...
- I will use Claude Code, but it should also work quite well with GitHub Copilot, Cursor, Windsurf, and others.
   - I do not provide the license; it currently costs €18 per month for the Pro plan.
...
- Clone this repository from GitHub: [https://github.com/AlbanOtt/chemometrics_vibe_coding](https://github.com/AlbanOtt/chemometrics_vibe_coding)
...
## What We Will NOT Cover Today 🙈 {.smaller}
...
- It’s not pipelining or automation.
...
Also, you are less constrained by the developer’s pipeline and much more flexible, especially regarding the input. 
...
If you already know these by heart, you are among the top 10% of high-quality Python developers... please leave; you will learn nothing here!!! 🤣
...
If you’re not familiar with [Quarto](https://quarto.org/), it is the successor to RMarkdown. In my opinion, it is far superior to Jupyter. Even these slides were created using Quarto.

Want to learn more?  
Please visit Mickaël Canouil’s course, "Mastering Quarto CLI: From Authoring to Publishing," at [https://m.canouil.dev/mastering-quarto-cli/](https://m.canouil.dev/mastering-quarto-cli/)

The [report](../reports/_book/index.html).

::: {.callout-note}
🧠 During the tour, please ask yourself:
...
[2  Metabolomics Data Preprocessing](../reports/_book/preprocessing_tutorial.html).

See "Human notes":

- [Bad coding practice](../reports/_book/preprocessing_tutorial.html#sec-blank-filtering)
- [Bad imputation practice](../reports/_book/preprocessing_tutorial.html#step-4-zero-handling)
- [PQN vs autoscale](../reports/_book/preprocessing_tutorial.html#step-5-pqn-normalization)

## EDA

- The [Timepoint PCA](../reports/_book/eda_tutorial.html) with Plotly really helps with interpretation.

## Compound Detection

- You should not only review the Quarto file but also the [.py file](https://github.com/AlbanOtt/chemometrics_vibe_coding/blob/7ddbef49031318cdd1f0cdbd92935640bbd40bf3/src/drug_detection.py#L22-L24).
...
- We can find N-desmethyl-DPH [4312](../reports/_book/multivariate_tutorial.html#fig-vip-4) in the VIP.
- We can find DPH N-oxide [5053](../reports/_book/multivariate_tutorial.html#fig-vip-4) in the VIP.
- We can find Diphenhydramine [4589](../reports/_book/multivariate_tutorial.html#fig-vip-4) in the VIP.
...
| **Validation**    | When comparable, results closely match both the original paper and summer school outcomes | — |
...
Select model                                                                                                                                                                                     
 ...
 ▌▌▌ Effort not supported for Sonnet
...
Opus 4.6 burns through your credits like crazy 🔥. For the workshop, I don’t want to run out of credits, so I’m using medium effort.  
...
>❯ \@data/CD-MRG-metabolomics_summerschool_2023/plan.md suggest another analysis I could do with this dataset 
>  ⎿  Read data\CD-MRG-metabolomics_summerschool_2023\plan.md (162 lines)
>
>∴ Thinking…
>
...
>  added.
>
>● Let me explore what's already been implemented to understand the gaps and opportunities.
>
>● Explore(Explore existing analyses)\
>⎿  Prompt:
...
>
>❯ Explore the codebase to understand what analyses have already been implemented for this metabolomics project. 
:::
...
[Phase 6 plan](https://github.com/AlbanOtt/chemometrics_vibe_coding/blob/a25bf12d39c18d10b0c62668cca1457ef13f128e/data/CD-MRG-metabolomics_summerschool_2023/plan_phase6.md)
...
[Report](../reports/_book/index.html)
...
- Skills
 - `quarto-authoring` Full documentation on Quarto, by Mickaël Canouil
...
```
The [CLAUDE.md](https://github.com/AlbanOtt/chemometrics_vibe_coding/blob/1cfc052b8a3ce524c454feac0f21d009d6080c4c/CLAUDE.md) I wrote for this workshop.
...
## Let's Write One Together! 🖊️ {.smaller}
...
These instructions, I provided them in the initial [prompt](https://github.com/AlbanOtt/chemometrics_vibe_coding/blob/feat/final_repos_state/data/CD-MRG-metabolomics_summerschool_2023/prompt.md) because I wanted to have a user experience like:
...
- `data/preprocessing_result.pkl`: Preprocessed metabolomics data
  ...
  - Features: m/z values (ppm tolerance: ±5 ppm)
...
❌ DO NOT use blank subtraction without careful review
❌ DO NOT use autoscaling (use PQN normalization instead)
❌ DO NOT fit preprocessing on full dataset before analysis
❌ DO NOT ignore mass accuracy and retention time variation
❌ DO NOT rely on statistical significance alone (check biological relevance)
❌ DO NOT report false compounds (verify with GNPS MS/MS data)

✅ Always document feature matching tolerance criteria
✅ Always check multivariate model diagnostics
✅ Always generate dummy datasets with known, controlled effects as a safeguard
```

::: {.callout-caution}
I haven't tested this already: `✅ Always generate dummy datasets with known, controlled effects as a safeguard.`
...
## 🎯 Check Your Understanding: CLAUDE.md {.smaller .flashcards showFlipButton=true}
...
I both well know Mickaël's and Posit:  
...
Result here:
[Skill](https://github.com/AlbanOtt/chemometrics_vibe_coding/blob/1cfc052b8a3ce524c454feac0f21d009d6080c4c/.claude/skills/chemometrics-ms-metabolomics/SKILL.md) based on Boccard & Rudaz, 2018
...
- We don't care about [references](https://github.com/AlbanOtt/chemometrics_vibe_coding/blob/2c833e601147947a098fa981dd3409e274f16a2f/.claude/skills/chemometrics-ms-metabolomics/SKILL.md#references) in the skill, it's a waste of token. Initial prompt should be improved. 
- Very low degree of freedom for the [multivariate analysis](https://github.com/AlbanOtt/chemometrics_vibe_coding/blob/2c833e601147947a098fa981dd3409e274f16a2f/.claude/skills/chemometrics-ms-metabolomics/references/multivariate-analysis.md). In the whole skill in general. I should improve my prompt.

- Boccard & Rudaz are very vague about missing values, still the skill has a [list of method](https://github.com/AlbanOtt/chemometrics_vibe_coding/blob/2c833e601147947a098fa981dd3409e274f16a2f/.claude/skills/chemometrics-ms-metabolomics/references/normalization.md#missing-value-handling), not what I would recommend. 
...
:::
::: {.fragment}
We have just designed a workflow—that is, a subagent.
:::
...
## 🎯 Check Your Understanding: Skills {.smaller .flashcards showFlipButton=true}
...
::: {.flashcard-back}
Yes — Claude should not limit itself to one skill.
:::

## 🎯 Check Your Understanding: Skills 2 {.smaller .flashcards showFlipButton=true}
...
## 🎯 Check Your Understanding: Skills 3 {.smaller .flashcards showFlipButton=true}
...
::: {.flashcard-back}
High freedom — text-based guidelines when multiple approaches are valid.

Medium freedom — pseudocode/scripts with parameters when a preferred pattern exists.

Low freedom — specific scripts with no parameters when operations are fragile and consistency is critical.
:::
...
| **Account** | Minimum access rights — no admin, no production credentials |
...
- Review each tool call — especially `Bash` commands
...
- MCP servers can make **API calls** — verify what endpoints they contact
...
[https://github.com/AlbanOtt/chemometrics_vibe_coding](https://github.com/AlbanOtt/chemometrics_vibe_coding)
